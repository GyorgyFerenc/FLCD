...
static Errorable<Process> run(std::function<void()> func) {
    ...
    if (pipe(pipe_stdin)) return {Process_Error::PipeError};
    if (pipe(pipe_stdout)) return {Process_Error::PipeError};

    pid_t p = fork();
    if (p < 0) {
        // Fork failed
        return {Process_Error::ForkError};
    } else if (p == 0) {
        // child
        close(pipe_stdin[1]);
        dup2(pipe_stdin[0], 0);
    ...
    return {Core_Error::Correct, proc};
}
...
void join(Process p) {
    int waitstatus;
    waitpid(p.child_pid, &waitstatus, 0);
}
