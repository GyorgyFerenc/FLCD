...
Scanner create(std::string source){
    let scanner = Scanner {
        ...
        .source              = source,
        .integer_constant_fa = FA::read_from_file("integer_constant_fa.txt"),
        .identifier_fa       = FA::read_from_file("identifier_fa.txt"),
    };
...
void all(){                             
    while (true) {
        ...
        { // integer constant
            ...
            if (integer_constant_fa.accept(str)) {
                let index = table.constants.add(str);
                pif.push_back({
                    .kind = Token::Kind::Constant,
                    .str = str,
